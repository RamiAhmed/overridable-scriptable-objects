...
public class SerializableObjectGenerator : IIncrementalGenerator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (typeSymbol == null || !InheritsFrom(typeSymbol, "OverridableScriptableObject"))
            return null;
        ...
        foreach (var typeDeclaration in typeDeclarations)
        {
            ...
            var code = $$"""
                         ...
                         namespace {{namespaceName}}
                         {
                             ...
                             /// It contains fields for each non-UnityEngine.Object, JSON-serializable, public or internal instance field of <see cref="{{typeName}}"/>.
                             /// </summary>
                             [Serializable]
                             public class {{typeName}}_GeneratedSerializableData : ISerializableOverridableScriptableObject
                             {
                                 // Fields
                                 {{string.Join("\n\t\t", fields.Select(p => $"public {p.Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)} {p.Name};"))}}
    ...
    private static IEnumerable<IFieldSymbol> GetAllFields(ITypeSymbol typeSymbol)
    {
        ...
        while (currentType != null && currentType.SpecialType != SpecialType.System_Object)
        {
            var members = currentType
                ...
                .Where(t => t.GetAttributes().All(a => a.AttributeClass?.Name != "IgnoreOverridableFieldAttribute"))
    ...
    private static bool IsTypeSerializable(ITypeSymbol type)
    {
        ...
        if (type.GetAttributes().Any(a => a.AttributeClass?.Name == nameof(SerializableAttribute)))
            return true;
